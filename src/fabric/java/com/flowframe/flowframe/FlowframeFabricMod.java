...
public class FlowframeFabricMod implements ModInitializer {
    ...
    private final Set<UUID> seenPlayers = new java.util.HashSet<>();
    ...
    public FlowframeFabricMod() {
        ...
        this.dataManager = new DataManager(configDir.resolve("keepinv.json"));
        this.endPortalEnabled = false;
    }
        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            ...
            dispatcher.register(
                CommandManager.literal("endtoggle")
                    ...
                    .executes(context -> {
                        endPortalEnabled = !endPortalEnabled;
                        saveConfig();
                        Text state = endPortalEnabled ? Text.literal("ENABLED").formatted(Formatting.GREEN) : Text.literal("DISABLED").formatted(Formatting.RED);
    ...
    private void registerEvents() {
        ...
        ServerEntityWorldChangeEvents.AFTER_PLAYER_CHANGE_WORLD.register((player, origin, destination) -> {
            if (!endPortalEnabled && destination.getRegistryKey() == World.END) {
    ...
    private void saveConfig() {
        // Persisted state is currently just the toggle flag; the keep-inventory file is handled by DataManager.
    }
    ...
    private void handleHeadRiding(net.minecraft.server.MinecraftServer server) {
        ...
        while (iterator.hasNext()) {
            ...
            double mountY = target.getY() + target.getHeight() + 0.25D;
            rider.requestTeleport(target.getX(), mountY, target.getZ());
            rider.setVelocity(0.0D, 0.0D, 0.0D);
        }
    }
