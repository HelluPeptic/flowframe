...
public final class DiscordIntegrationMod {
    ...
    /**
     * Called by external mods (e.g., Flowframe) to relay custom chat messages to Discord.
     */
    public static void onFlowframeChat(ServerPlayer player, String message) {
        ...
        if (!Localization.instance().discordChatMessage.isBlank()) {
            if (Configuration.instance().embedMode.enabled && Configuration.instance().embedMode.chatMessages.asEmbed) {
                ...
                DiscordIntegration.INSTANCE.sendMessage(ArchitecturyMessageUtils.formatPlayerName(player), player.getUUID().toString(), new DiscordMessage(null, text, true), channel);
            }
        }
    }
}
